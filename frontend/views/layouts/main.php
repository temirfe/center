...
if (!isset($user_id) && $identity) $user_id = $identity->id;
else $user_id = '';
if (!isset($user_name) && $identity) $user_name = $identity->username;
else $user_name = '';
if (!isset($user_role) && $identity) $user_role = $identity->role;
else $user_role = '';
if (!isset($dao)) $dao = Yii::$app->db;

if ($controller == 'site' && $action == "about") $about_active = true;
else $about_active = false;
if ($controller == 'expert') $expert_active = true;
else $expert_active = false;
if ($controller == 'article') $article_active = true;
else $article_active = false;
if ($controller == 'event') $event_active = true;
else $event_active = false;
if ($controller == 'video') $video_active = true;
else $video_active = false;
if ($controller == 'site' && $action == "partners") $partner_active = true;
else $partner_active = false;
if ($controller == 'project') $proj_active = true;
else $proj_active = false;

$projects = Yii::$app->cache->getOrSet('project', function () {
    return Project::find()->all();
}, 0);
$projItems = [];
foreach ($projects as $dest) {
    if ($projItems) {
        $projItems[] = '<li class="divider"></li>';
    }
    $projItems[] = ['label' => $dest->title, 'url' => ['/project/view', 'id' => $dest->id, 't' => $dest->title]];
}
?>
<?php $this->beginPage() ?>
<!DOCTYPE html>
...
<body data-spy="scroll" data-target="#myScrollspy" data-offset="140" id="top">
    ...
    <div class="wrap">
        <?php
        if (!$isGuest && $user_role == 'admin') {
            include_once('_adminpanel.php');
        }
        ...
        $menuItems[] = ['label' => Yii::t('app', 'Partners'), 'url' => ['/partners'], 'active' => $partner_active];
        ...
        echo "<div class='pull-right mt10'>" . Html::a(
            Yii::t('app', 'Search') . "<span class='fa fa-search search_icon'></span>",
            ['/site/search'],
